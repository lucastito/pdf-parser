...
| [0021](0021-taxonomia-de-caracteristicas.md) | Característica estrutural é **2º eixo da página**, ao lado da classe de conteúdo — implementado via `diagnostico.caracterizar_pagina` | Uma página é `DADOS` **e** `DIGITALIZADA` ao mesmo tempo; rótulo por documento apagaria o **documento misto**, que é o caso difícil |
...
| [0023](0023-prompt-para-estrutura-desconhecida.md) | No produto, o prompt é **montado do diagnóstico**, não declarado por humano — implementado (Cenário B) | Genérico dá **0%**, regra descrita dá **60%**, ordem declarada dá **100%** — mas só funciona num documento por vez |
| [0024](0024-reconhecimento-antes-da-extracao.md) | O parser **reconhece** a página antes de extrair, em três níveis de custo — implementado (Cenário B) | Descrever a estrutura custa **9%** do que custa extraí-la; e geometria e modelo têm **pontos cegos complementares** |
| [0025](0025-roteador-de-extracao-por-pagina.md) | Rota decidida por página em tempo de execução, nunca declarada no perfil — implementado e em produção (Cenário B) | Liga ADR-0021/0023/0024 ao caminho de produção — `descobrir_nomes_de_coluna` sai de "só teste" para o fluxo real |
